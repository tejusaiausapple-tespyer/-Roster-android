...
>
> **Update 2026-07-24**: Item 7 added — Messages is now explicitly out of scope for Android entirely (not deferred). This same pass also re-verified items 2 and 6 fresh: item 2's bug is confirmed still live and unfixed in the PWA today; item 6's "structurally absent" finding was accurate for *manager* Messages specifically, but a broader re-check found iOS's *staff-side* Messages inbox (`NotificationsSheet`) is genuinely live, not absent — that nuance didn't matter for item 6's original manager-only scope, but is recorded here for accuracy since item 7 now makes it moot for Android either way.
...
6. ~~**Manager Messages on iOS**~~ — **RESOLVED: confirmed structurally absent, not just unfound.** An exhaustive grep of the entire iOS source tree found exactly one Firestore write path touching `messages` (mark-as-read); there is no `.setData`/`addDocument`/compose call anywhere, no `Features/Manager/Messages/` directory, and no such tab in `ManagerNavigation.swift`. Manager Messages must be built directly from the PWA's `MessagesPage` as the spec (with the correction in item 2 above) — no further iOS discovery-refresh is needed. **Superseded by item 7 below** — moot now that Messages isn't being built for Android at all.
7. ~~**Messages feature — build for Android at all?**~~ — **RESOLVED (product owner, 2026-07-24): not needed, for either iOS parity or Android.** This applies to the whole feature, not just the manager-compose half — including the staff-side inbox/read UI, which a 2026-07-24 re-verification confirmed *is* genuinely live on iOS (`NotificationsSheet`, contrary to how item 6 above reads in isolation) and *is* genuinely still fully live and unremoved in the PWA (nothing was actually pulled there — see item 2's "Never Expire" bug, still present, as the one live PWA-side issue). None of that changes this decision: no `MessageRepository`, no bell-badge unread-message count, no inbox UI, on Android. Phase G narrows to Daily Jobs only (Home bell badge = pending Daily Jobs count only) — see `ANDROID-STAFF-BUILD-PLAN.md` Phase G's own scope note for the staff-plan-level edits this drove. If Messages is ever reconsidered later, treat this as a fresh scoping conversation, not a resumption of the old plan — a lot of the "old plan" text elsewhere in this document (§3 checklist, Phase 8/9 scope) still describes Messages as an Android build target and is now stale; it wasn't rewritten line-by-line since Phases 8/9 are far from active, but this item is the authoritative override wherever those older mentions conflict with it.
